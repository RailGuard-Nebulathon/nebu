...
- [x] Root `app/` submission package mirrors the deployable frontend, backend, shared inference code, configuration, scripts, deployment files, documentation and tests while excluding predictions, checkpoints, datasets, dependencies, caches and build outputs.
- [x] Removed the top-right service/event labels, introductory console label and redundant workflow timeline; Start another analysis now appears as a prominent result-header action beside Export data.
