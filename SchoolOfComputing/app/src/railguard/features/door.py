...
class DoorFeatureExtractor(CombinedFeatureExtractor):
    ...
    def _one(self, sample: SequenceSample) -> dict[str, float]:
        ...
        channels = {
            name: sample.values[:, i].astype(float) for i, name in enumerate(sample.channel_names)
        }
        ...
        correlation = np.correlate(
            current - current.mean(), velocity - velocity.mean(), mode="full"
        )
        lag = int(np.argmax(np.abs(correlation)) - (len(current) - 1))
        operation = str(sample.metadata.get("operation", "Unknown"))
        output.update(
            {
                "door__operation_open": float(operation == "Open"),
                "door__operation_close": float(operation == "Close"),
                "door__total_travel": float(np.abs(position[-1] - position[0])),
                "door__monotonicity_violations": float(
                    np.sum(np.sign(velocity[1:]) != np.sign(np.median(velocity)))
                ),
                "door__average_velocity": float(np.mean(np.abs(velocity))),
                "door__max_velocity": float(np.max(np.abs(velocity))),
                "door__average_acceleration": float(np.mean(np.abs(acceleration))),
                "door__max_acceleration": float(np.max(np.abs(acceleration))),
                "door__stall_fraction": float(
                    np.mean(position_delta < max(np.quantile(position_delta, 0.1), 1e-9))
                ),
                "door__power_proxy_mean": float(np.mean(power_proxy)),
                "door__energy_proxy": float(np.trapezoid(power_proxy)),
                "door__current_per_position_change": float(
                    np.sum(np.abs(current)) / max(np.sum(position_delta), 1e-9)
                ),
                "door__current_position_velocity_correlation": float(
                    np.corrcoef(current, velocity)[0, 1]
                )
                if np.std(velocity) > 0 and np.std(current) > 0
                else 0.0,
