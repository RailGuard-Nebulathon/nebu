...
from railguard.data.adapters.door import CONTINUOUS_CHANNELS, STATE_CHANNELS, infer_cycle_boundaries, validate_door_schema
...
class RailGuardPredictor:
    ...
    def _frame(self, input_path: str | Path) -> tuple[pd.DataFrame, list[dict[str, object]]]:
        ...
        if self.task == "door":
            raw = pd.read_csv(path); validate_door_schema(raw); timestamps, errors = parse_door_timestamps(raw["Datetime"], strict=False)
            ...
            for number, (start, end, operation) in enumerate(infer_cycle_boundaries(raw), 1):
                samples.append(SequenceSample(f"segment_{number}", raw.iloc[start : end + 1][channels].to_numpy(np.float32), timestamps[start : end + 1], channels, {"operation": operation}))
                records.append({"sample_id": f"segment_{number}", "start_time": raw.iloc[start]["Datetime"], "end_time": raw.iloc[end]["Datetime"]})
            return door_feature_table(samples), records
        if self.task == "acv":
            raw = pd.read_excel(path); _, cars = split_car_columns(raw)
            frame = acv_candidate_features(raw, path.name)
            return frame.drop(columns=["case_id", "car_id"]), [{"sample_id": car, "file_id": path.name} for car in frame["car_id"]]
        if self.task == "corrugation":
            raw = pd.read_csv(path); channels = [str(c) for c in raw if c != SPEED_COLUMN]
            sample = SequenceSample(path.name, raw[channels].to_numpy(np.float32), None, channels, {"sampling_rate_hz": 10000})
            return pd.DataFrame([corrugation_features(sample)]), [{"sample_id": path.name, "file_id": path.name}]
        if self.task == "shm":
            raw = pd.read_csv(path, header=None); channels = [f"stress_{i}" for i in range(raw.shape[1])]
            sample = SequenceSample(path.name, raw.to_numpy(np.float32), None, channels)
            return pd.DataFrame([shm_features(sample)]), [{"sample_id": path.name, "file_id": path.name}]
    ...
    def predict(self, task: str, input_path: str | Path) -> list[Prediction]:
        ...
        classes = list(self.model.pipeline.named_steps["model"].classes_) if probabilities is not None else []
        return [Prediction(
            sample_id=str(records[i]["sample_id"]), task=task, prediction=value,
            probabilities={str(label): float(probabilities[i, j]) for j, label in enumerate(classes)} if probabilities is not None else None,
            confidence=float(probabilities[i].max()) if probabilities is not None else None, metadata=records[i],
