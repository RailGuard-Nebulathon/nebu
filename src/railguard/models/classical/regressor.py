...
def _estimator(name: str, seed: int) -> Any:
    choices = {
        "ridge": Ridge(alpha=1.0), "elastic_net": ElasticNet(alpha=0.01, random_state=seed),
...
class ClassicalRegressor:
    def __init__(self, name: str = "extra_trees", seed: int = 42, target_transform: str = "raw") -> None:
        ...
        self.pipeline = Pipeline([("imputer", SimpleImputer(strategy="median")), ("scale", StandardScaler()), ("model", _estimator(name, seed))])
    ...
    def predict(self, x: pd.DataFrame | np.ndarray) -> np.ndarray:
        ...
        return np.expm1(prediction) if self.target_transform == "log1p" else prediction

    def calibrate_intervals(self, x: pd.DataFrame | np.ndarray, y: Any, coverage: float = 0.9) -> None:
        self.residual_quantile_ = float(np.quantile(np.abs(np.asarray(y) - self.predict(x)), coverage))
    ...
    def metadata(self) -> dict[str, Any]:
        return {"kind": "regression", "model": self.name, "seed": self.seed, "target_transform": self.target_transform, "feature_names": self.feature_names_, "checkpoint_version": 1}
