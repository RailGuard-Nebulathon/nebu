...
def spectral_features(values: np.ndarray, sampling_rate: float, bands: int = 4) -> dict[str, float]:
    ...
    total = float(np.trapezoid(power, frequencies)) if len(frequencies) > 1 else 0.0
